...
app.post('/Weather', express.urlencoded({extended: true}), function(req, res) {
  ...
  const https = require('https');
  https.get('https://api.openweathermap.org/data/2.5/weather?lat=' + latitude + '&lon=' + longitude + '&appid=c5c81d1d2b51807c753d34b4ab9227db', (resp) => {
    let data = '';
...
app.post('/WeatherForecast', express.urlencoded({extended: true}), function(req, res) {
  ...
  https.get('https://api.openweathermap.org/data/2.5/forecast?lat=' + latitude + '&lon=' + longitude + '&appid=c5c81d1d2b51807c753d34b4ab9227db', (resp) => {
    let data = '';
    // A chunk of data has been recieved.
    resp.on('data', (chunk) => {
      data += chunk;
